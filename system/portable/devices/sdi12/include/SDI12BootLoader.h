...
public:
  ...
  static void base64out(unsigned char ch);
  static unsigned char base64in(unsigned char ch);
  static unsigned char hexIn(unsigned char ch);
  static void storeOkUnsigned(unsigned short w);
  static bool processCommand(void);
  static void processDelayedActions(void);
  static void taskMain(void) __attribute__( ( noreturn ) );
