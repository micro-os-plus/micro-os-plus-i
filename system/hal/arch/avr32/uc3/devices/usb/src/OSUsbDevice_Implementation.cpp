...
OSUsbDeviceImpl::usbDriverInit(void)
{
...
#if OS_CFGLONG_OSCILLATOR_HZ == 12000000L
  ...
  bool
  OSUsbDeviceImpl::usb_configure_endpoint(unsigned char num,
      unsigned char type, unsigned char dir, unsigned char size,
      unsigned char bank, unsigned char nyet)
  {
    nyet = nyet;
  ...
  OSUsbDeviceImpl::usb_init_device(void)
  {
    return AVR32_is_usb_id_device()
        && !AVR32_is_usb_endpoint_enabled(EP_CONTROL)
   ...
   */
  bool
  OSUsbDeviceImpl::specificGetDescriptor(unsigned char type,
    ...
    while (1)
      {
        ...
        if (wIndex != ((S_usb_interface_descriptor*) pbuffer)->bInterfaceNumber)
          continue;
        if (wValue
            != ((S_usb_interface_descriptor*) pbuffer)->bAlternateSetting)
          continue;
    ...
    while (1)
      {
        ...
        pbuffer = (U8*) pbuffer
            + ((S_usb_endpoint_descriptor*) pbuffer)->bLength;
        if (INTERFACE_DESCRIPTOR
        ...
        if (ENDPOINT_DESCRIPTOR
            == ((S_usb_endpoint_descriptor*) pbuffer)->bDescriptorType)
          {
            // Reset endpoint
            u8_i
                = ((S_usb_endpoint_descriptor*) pbuffer)->bEndpointAddress
                    & (~MSK_EP_DIR);
            endpointSelect(u8_i);
            Usb_disable_stall_handshake();
            endpointReset(u8_i);
      ...
      usb_conf_desc_fs =
            {
                  ...
                  { sizeof(S_usb_interface_descriptor), INTERFACE_DESCRIPTOR,
                      IF0_NB, ALTERNATE0, NB_ENDPOINT0, IF0_CLASS,
                      IF0_SUB_CLASS, IF0_PROTOCOL, STRING_INDEX_IF0 },
