 ...
 *
 * In addition, special care should be given to starting the first task, that
...
OSSchedulerImpl::FirstTask_contextRestore(void)
{
  ...
  (
      ...
      " ld.w    r0, sp[-8*4] \n"
      " mtsr    %[SR], r0 \n"
      // From this moment on, the interrupt mask is restored to initial value,
      // so interrupts might be enabled.
...
SCALL_contextRestore(void)
{
  ...
  (
      ...
      // SP must be left at bottom of stack, otherwise, in case
      // interrupts occur, the stack content will be trashed.
