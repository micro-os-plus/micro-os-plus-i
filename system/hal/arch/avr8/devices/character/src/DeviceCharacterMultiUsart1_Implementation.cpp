/*
 *	Copyright (C) 2007-2011 Liviu Ionescu.
 *
 *	This file is part of the uOS++ distribution.
 */

#include "portable/kernel/include/OS_Defines.h"

// DEPRECATED

#if false && defined(OS_INCLUDE_DEVICECHARACTERMULTIUSART1)

#include "portable/devices/character/include/DeviceCharacterMultiUsart1.h"

#if defined(OS_CONFIG_DEVICECHARACTERMULTIUSART1_BAUD_CONSTANT)
#define DEVICECHARACTERMULTIUSART1_BAUD_CONSTANT     OS_CONFIG_DEVICECHARACTERMULTIUSART1_BAUD_CONSTANT
#else

#if (OS_CONFIG_DEVICECHARACTERMULTIUSART1_DOUBLE_SPEED)
#define DEVICECHARACTERMULTIUSART1_PRESCALLER        (8)
#else
#define DEVICECHARACTERMULTIUSART1_PRESCALLER        (16)
#endif

#define DEVICECHARACTERMULTIUSART1_CLOCK_HZ          (OS_CFGLONG_OSCILLATOR_HZ / OS_CFGINT_CLOCK_PRESCALER / DEVICECHARACTERMULTIUSART1_PRESCALLER)
#define DEVICECHARACTERMULTIUSART1_BAUD_CONSTANT     ((DEVICECHARACTERMULTIUSART1_CLOCK_HZ / OS_CONFIG_DEVICECHARACTERMULTIUSART1_BAUD_RATE)- 1UL)

#endif

// ---------------------------------------------------------------------------

void DeviceCharacterMultiUsart1::implPortInit(void)
  {
    OSScheduler::criticalEnter();
      {
        OSDeviceDebug::putString("Baud constant=");
        OSDeviceDebug::putDec(
            ( unsigned short ) DEVICECHARACTERMULTIUSART1_BAUD_CONSTANT, 0);
        OSDeviceDebug::putNewLine();
      }
    OSScheduler::criticalExit();

    /* Set the baud rate. */
    UBRR1 = DEVICECHARACTERMULTIUSART1_BAUD_CONSTANT;

#if (OS_CONFIG_DEVICECHARACTERMULTIUSART1_DOUBLE_SPEED)
    UCSR1A = _BV(U2X1);
#else
    UCSR1A = 0;
#endif
    /* Enable the Rx interrupt.  The Tx interrupt will be enabled
     later. Also enable the Rx and Tx. */
    UCSR1B = _BV(RXCIE1) | _BV(RXEN1) | _BV(TXEN1);

    /* Set 2-bit stop and the data bits to 8. */
    UCSR1C = ( ( 1 << USBS1 ) | ( 3 << UCSZ10 ));
  }

unsigned char DeviceCharacterMultiUsart1::implPortRead(void)
  {
    return UDR1;
  }

void DeviceCharacterMultiUsart1::implPortWrite(unsigned char b)
  {
    UDR1 = b;
  }

void DeviceCharacterMultiUsart1::implInterruptTxEnable(void)
  {
    UCSR1B |= _BV(UDRIE1);
  }

void DeviceCharacterMultiUsart1::implInterruptTxDisable(void)
  {
    UCSR1B &= ~_BV(UDRIE1);
  }

// ===== ISRs ================================================================

#if (OS_CFGBOOL_DEVICECHARACTERMULTIUSART1_TX_CONTEXT_SWITCH)

extern "C"
void  USART1_UDRE_vect( void ) __attribute__( ( signal, naked ) );

void USART1_UDRE_vect(void)
  {
    // interrupts disabled in here
    OSScheduler::interruptEnter();
      {
        ((DeviceCharacterMultiUsart1 *)DeviceCharacterMultiUsart1::ms_pThis)->interruptTxServiceRoutine();
      }
    OSScheduler::interruptExit();
    // interrupts enabled after this point
  }

#else

extern "C"
void USART1_UDRE_vect( void ) __attribute__( ( signal ) );

void USART1_UDRE_vect(void)
  {
    OSScheduler::ledActiveOn();

    ((DeviceCharacterMultiUsart1 *)DeviceCharacterMultiUsart1::ms_pThis)->interruptTxServiceRoutine();
  }

#endif /*OS_CFGBOOL_DEVICECHARACTERMULTIUSART1_TX_CONTEXT_SWITCH*/

#if (OS_CFGBOOL_DEVICECHARACTERMULTIUSART1_RX_CONTEXT_SWITCH)

extern "C"
void  USART1_RX_vect( void ) __attribute__( ( signal, naked ) );

void USART1_RX_vect(void)
  {
    // interrupts disabled in here
    OSScheduler::interruptEnter();
      {
        ((DeviceCharacterMultiUsart1 *)DeviceCharacterMultiUsart1::ms_pThis)->interruptRxServiceRoutine();
      }
    OSScheduler::interruptExit();
    // interrupts enabled after this point
  }

#else

extern "C"
void USART1_RX_vect( void ) __attribute__( ( signal ) );

void USART1_RX_vect(void)
  {
    OSScheduler::ledActiveOn();

    ((DeviceCharacterMultiUsart1 *)DeviceCharacterMultiUsart1::ms_pThis)->interruptRxServiceRoutine();
  }

#endif /*OS_CFGBOOL_DEVICECHARACTERMULTIUSART1_RX_CONTEXT_SWITCH*/

#endif /*OS_INCLUDE_DEVICECHARACTERMULTIUSART1*/
